...
module.exports = {
    ...
    fatal: function (message, data) {
        this.format(message, data, "Fatal");
    },

    error: function (message, data) {
        if (this.threshold === "fatal") { return; }
        this.format(message, data, "Error");
    },

    warning: function (message, data) {
        if (["fatal", "error"].indexOf(this.threshold) >= 0) { return; }
        this.format(message, data, "Warning");
    },

    format: function (message, data, label) {
        this.io.puts("[" + label + "] " + message);
        if (data && data.errors) {
